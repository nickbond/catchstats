...
download_awap <- function() {
   # loc_dir <- ifelse(!is.null(loc_dir), loc_dir, getwd())
    ...
    hist_remote_file_names <- RCurl::getURL("ftp://ftp-rs.it.csiro.au/pub/awap/Australia_historical/Run26j/FWDis/", verbose = TRUE, dirlistonly = TRUE, ftp.use.epsv = TRUE)
    op_remote_file_names <- RCurl::getURL("ftp://ftp-rs.it.csiro.au/pub/awap/Australia_operational_v26/monthly/flt-zip/", verbose = TRUE, dirlistonly = TRUE, ftp.use.epsv = TRUE)
    ...
    for (i in 1:length(updated.local.files)) {
        utils::unzip(updated.local.files[i], exdir = file.path("awap_raw_data"), junkpaths = TRUE)
    }
}
