...
extractRasterPoly_cl <- function(shpfile, rast, catid_col, fun = mean, weights = TRUE, normalizeWeights = TRUE, na.rm=TRUE, small = TRUE, start = 1, end = nrow(shpfile)) {
  ...
  fits<-parallel::parLapplyLB(cl,1:length(shpfile),function(i) loc.values[[i]] <- raster::extract(rast, poly, na.rm = na.rm, weights = weights, fun = fun, normalizeWeights = normalizeWeights, small = small)
