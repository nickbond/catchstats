...
#  ds.sites<-unique(c(ds.sites,site1,site2))
  ...
  #if(env.vars)
 #   slope<-hierarchy[match(ds.sites, hierarchy$site), 'slope']
 # rd<-hierarchy[match(ds.sites, hierarchy$site), 'length']
...
#clus<-makeCluster(8)
#clusterExport(clus, c("networkstats", "list.all.downstream", "alldownstream", "hierarchy"))
#system.time(test<-parRapply(clus, all.pairs, function(x) networkstats(x[1],x[2])))
